...
- For ledger corrections: the task tells you exactly what the errors are — trust it. Fetch postings for the relevant accounts in ONE batched call to find the voucher IDs. Before correcting, fetch the full voucher (GET /ledger/voucher/{id} with postings) to see its structure — counter-accounts, supplier refs, VAT types. The corrected voucher must preserve the original structure (same counter-account, same refs) with only the specific error fixed.
